...
class OrangeTree
  ...
  def initialize
    ...
    @age = 0
    @dead = @@dead
    @fruit_bearing_age = @@fruit_bearing_age
    @oranges = []
  ...
  def any_oranges?
     if @oranges.empty? #it will return true if array is empty but condition will return false 
       return false
      end
      return true
  ...
  def pick_an_orange!
   ...
   raise NoOrangesError , "There is no orange" unless self.any_oranges?
